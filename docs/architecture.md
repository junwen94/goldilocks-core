...
| Assets              | `assets/`: installation and integrity; `ml/models.py`: model declarations; `assets/pseudopotentials/registry.py` and `assets/pseudopotentials/importers.py`: table declarations and provider normalization; `pseudo/source.py`: selection and deferred publication material; `pseudo/pp_registry.py`: user-supplied local UPF directories. |
